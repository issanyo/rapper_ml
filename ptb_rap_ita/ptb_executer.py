...
def generate_text(session, model, word_to_index, index_to_word, seed='</s>', n_sentences=10):
  sentence_cnt = 0
  seedSplit = re.compile(r"\W").split(seed)
  input_seeds_id = [word_to_index[w] for w in seedSplit]
  state = session.run(model.initial_state)

  # Initiate network with seeds up to the before last word:
  for x in input_seeds_id:#input_seeds_id[:-1]:
  ...
  input_wordid = input_id
  while sentence_cnt < n_sentences:
    feed_dict = {model.input.input_data: input_wordid, model.initial_state: state}
    probas, state = session.run([model.probas, model.final_state], feed_dict=feed_dict)
    sampled_word = sample_from_pmf(probas[0])

    if sampled_word in index_to_word:
      if sampled_word == word_to_index['eor']:
        text += '---'
      elif sampled_word == word_to_index['eos']:
        text += '\n'
        sentence_cnt += 1
      else:
...
def main(_):
  ...
  while True:
    generate_text(sess, mtest, word_to_id, id_to_word, seed="dottori", n_sentences=100)
    try:
